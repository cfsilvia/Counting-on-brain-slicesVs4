...
from get_slice_labels import get_slice_labels
import cv2

class Class_Manager_Counting:
    
    def __init__(self, Folder_with_data,Type_of_staining,scale):
        self.Folder_with_data =  Folder_with_data
        self.Type_of_staining = Type_of_staining
        self.scale = scale
    '''
    input: staining and input folder
    output: dictionary for each staining with the file_names 
    '''
    ...
    def ManagerSecondStep(self,Staining_dict):
        count_file = 0
        df_all_files = pd.DataFrame()
        for f in Staining_dict[self.Type_of_staining[0]]:
          try:
            filename, number_slice = Class_Manager_Counting.getFileInf(f)
            instance_roi = Read_Roi_imagej(f)
            list_roi_names, list_roi_coord = instance_roi()
        
            # for each staining get the labels
            instance_labels = get_slice_labels(self.Folder_with_data,filename, self.Type_of_staining)
            labels_dict = instance_labels()
            #%%
            df_total = Class_Manager_Counting.ManagerThirdStep(list_roi_names, list_roi_coord ,Staining_dict,count_file,
                                                               self.Type_of_staining,self.Folder_with_data,labels_dict, self.scale)
    ...
    '''
    @staticmethod
    def ManagerThirdStep(list_roi_names, list_roi_coord ,Staining_dict,count_file,Type_of_staining,Folder_with_data,labels_dict,scale,
                         ):
        count_rois = 0
        for rois in list_roi_names:
            #counting of each staining
            labels, area_roi_pixel, labels_dict = Class_Manager_Counting.ManagerFourStep(list_roi_coord[count_rois],Type_of_staining, labels_dict)
            
            #add intersections between stainings
            labels_dict, labels = Class_Manager_Counting.ManagerFiveStep(labels_dict, labels, Type_of_staining)
            
            #%% addition
            labels['roi_area_mmxmm'] = area_roi_pixel*(648.4e-06)**2 #mmxmm
            labels['roi_name'] = rois 
            #%% Save labels coordinates for further use
            Class_Manager_Counting.SaveRois(labels_dict,rois,Staining_dict[Type_of_staining[0]][count_file],Folder_with_data)
            #%%
    ...
    '''  
     ...
     '''       
    @staticmethod 
    def ManagerFourStep(roi_coord,Type_of_staining,labels_dict): 
       
        number_labels ={}
        polygons_inside_roi = {}
        
        for staining in Type_of_staining:
            number_labels[staining],area_roi_pixel, polygons_inside_roi[staining] = Class_CountingLabels.FindLabels_WithinMask(roi_coord, labels_dict[staining])
            
        return  number_labels, area_roi_pixel, polygons_inside_roi  
    ...
    #%% Create 2 folders for saving 
        file_name_without_extension,aux = Class_Manager_Counting.getFileInf(path_filename)
    #%% Create 2 folders for saving
        Class_Manager_Counting.CreateNewFolders(Folder_with_data + 'Roi_of_labels/' + file_name_without_extension + '/' )
    ...
    def __call__(self):
         ...
         df_all_files = self.ManagerSecondStep(Staining_dict)
         #%% save in an excel file 
         #%%Create folder to save
         Class_Manager_Counting.CreateNewFolders(self.Folder_with_data + 'Counting_Results/')
