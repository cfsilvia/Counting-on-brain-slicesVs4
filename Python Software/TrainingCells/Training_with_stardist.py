...
def Training(input_directory, model_name, dir_for_models):
    np.random.seed(42)
    lbl_cmap = random_label_cmap()

    # %% Input data
    X = sorted(glob(input_directory + '/images/*.tif'))
    Y = sorted(glob(input_directory + '/masks/*.tif'))
...
# %% #Normalize images and fill small holes
...
#%%    #%% Note: The trained StarDist2D model will not predict completed shapes for partially visible objects at the image boundary if train_shape_completion=False (which is the default option).
        model = StarDist2D(conf, name= 'stardist_test', basedir= dir_for_models)
...
#%%
    ...
    if not quick_demo:
        model.export_TF(fname=dir_for_models + model_name + '.zip')