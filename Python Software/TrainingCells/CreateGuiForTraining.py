...
def on_ok_button():
    if variable.get() == "Plot for a given file the counts and density of each region":
        ...
        messagebox.showinfo("NOTE",
                            "SAVE IN THE SAME FOLDER THE EXCELS YOU WANT TO COMPARE,\n Add to the name of each file  _type as _old")
...
def create_gui():
    ...
    options = ["Plot for a given file the counts and density of each region",
               "Compare the counts and density of several samples ", "Get Summarize results in excel file",
               "Get Summarize results in plot bar", "Get Summarize results in heat map",
               "Get Table with significant parameters"]
